...
struct VROP
{
  ...
  struct Textured3D_Blend
  {
                            ...
                            int* pLineColor, float* pLineDepth)
    {
      ...
      const vfloat c_one   = splat(1.0f);
      const vfloat c_255   = splat(255.0f);
      const vfloat inv_255 = splat(1.0f / 255.0f);
      ...
      if(test_bits_any(zTest))
      {
        ...
        const vfloat colorNewX = r * texColor[0];
        const vfloat colorNewY = g * texColor[1];
        const vfloat colorNewZ = b * texColor[2];
        const vfloat colorNewW = a * texColor[3]; //#TODO: try put c_ne here; try put half ...
        ...
        const vint colori = (to_int32( (colorOldX*one_minus_alpha + colorNewW*colorNewX)*c_255 ) << 16) | // BGRA
                            (to_int32( (colorOldY*one_minus_alpha + colorNewW*colorNewY)*c_255 ) << 8)  |
                            (to_int32( (colorOldZ*one_minus_alpha + colorNewW*colorNewZ)*c_255 ) << 0)  |
                            (to_int32( (colorOldW*one_minus_alpha + colorNewW*colorNewW)*c_255 ) << 24);
